...
function CoordenadasApp() {
    ...
    useEffect(() => {
        const entrenarModelo = async () => {
            ...
            await model.fit(datosEntrenamiento, etiquetas, {
                ...
                batchSize: 32,
                callbacks: {
                    onEpochEnd: (epoch, logs) => console.log(`Epoch ${epoch + 1}: Loss = ${logs.loss} Accuracy: ${logs.acc.toFixed(4)}`)
                }
    ...
    const calcularCalorias = (distancia) => {
        ...
        const factorCalorico = 0.5; // Calorías por km por kg de peso (ajustado para pruebas)
